...
function genCategories(engineRoot, categoryFolder) {
  ...
  if (!rows) {
    let dbFiles = [];
    if (fs.existsSync(dbDir)) dbFiles = fs.readdirSync(dbDir).filter(f => /\.db$/i.test(f));
    // DBC 旧格式（非 SQLite）——老 GOM 端常见，明确提示避免误以为工具坏了
    return { ok: false, msg: dbFiles.length
      ? '物品库是 DBC 旧格式（' + dbFiles.join(', ') + '，非 SQLite），暂不支持分类生成；请用 M2 数据管理工具导出为 SQLite（ApexM2.db）后再生成'
      : '未找到可读的 SQLite 物品库（Mud2/DB/*.db 含 StdItems 表；GUI 需系统 node 或本机 node.exe）' };
  }
  const out = {};
  ...
  for (const r of rows) {
    ...
    if (!name) continue;
    const m = (r.StdMode !== undefined ? r.StdMode : r.Stdmode), sh = r.Shape || 0, id = r.Idx;
    let cat = null;
...
// #CALL 递归掉落解析（蜗牛/定制爆率体系共享：MonItems/<怪>.txt → 通用爆率/地图爆率/装备列表，多层嵌套）
// 返回 [{ item, den }]（den=最终概率分母 1/N，含 #CHILD 组概率累乘；同物品保留更小 den）
// genZoneData 取 item 去重生成物品数据；exportSiteData 取 item+den 生成开区网站掉落，保证两处一致
function resolveMonDropsAll(engineRoot, monName) {
  const itemsSrc = monItemsDir(engineRoot);
  const qdDir = path.join(engineRoot, 'Mir200', 'Envir', 'QuestDiary');
  const visited = new Set();
  const result = [];
  const idxMap = {};
  const extractSegment = (content, segName) => {
    const lines = content.split(/\r?\n/);
    const target = '[@' + segName + ']';
    let start = -1;
    for (let i = 0; i < lines.length; i++) { if (lines[i].trim() === target) { start = i + 1; break; } }
  ...
  const parseLines = (lines, groupStack) => {
    let curMul = groupStack.length ? groupStack[groupStack.length - 1] : 1;
    for (const line of lines) {
      const t = line.trim();
      if (!t || t.startsWith(';') || t === '{' || t === '}') continue;
      const callM = t.match(/^#CALL\s*\[\\?([^\]]+)\]\s*@(\S+)/i);
      if (callM) {
        const rel = callM[1].replace(/^[\\/]+/, '').replace(/\\+/g, '\\');
        const segName = callM[2];
        const fp = path.join(qdDir, rel);
        const key = fp + '|' + segName;
        if (fs.existsSync(fp) && !visited.has(key)) {
          visited.add(key);
          try {
            const seg = extractSegment(decodeBuf(fs.readFileSync(fp)), segName);
            if (seg) parseLines(seg, groupStack);
          } catch (e) { /* 单个 #CALL 解析失败忽略 */ }
        }
        continue;
      }
      if (t === '(') continue;
      if (t === ')') { if (groupStack.length) groupStack.pop(); continue; }
      if (t.startsWith('#')) {
        const cm = t.match(/^#CHILD\s+(\d+)\/(\d+)/i);
        if (cm) {
          const den = Math.max(1, Math.round(+cm[2] / +cm[1]));
      ...
      if (m) {
        let item = m[3].trim().replace(/\s+\d+$/, '').trim();
        if (item && !/[<>:"/\\|?*]/.test(item)) {
          const den0 = Math.max(1, Math.round(+m[2] / +m[1]));
          const effectiveDen = Math.max(1, Math.round(den0 * (groupStack.length ? groupStack[groupStack.length - 1] : 1)));
          if (idxMap[item] === undefined) { idxMap[item] = result.length; result.push({ item, den: effectiveDen }); }
          else if (effectiveDen < result[idxMap[item]].den) result[idxMap[item]].den = effectiveDen;
        }
      } else if (!/^[A-Za-z]/.test(t)) {
        const item = t.replace(/\s+\d+$/, '').trim();
        if (item && !/[<>:"/\\|?*]/.test(item) && idxMap[item] === undefined) { idxMap[item] = result.length; result.push({ item, den: 1 }); }
      }
    }
  };
  const f = path.join(itemsSrc, monName + '.txt');
  if (fs.existsSync(f)) {
    visited.add(f + '|' + monName);
    try { parseLines(decodeBuf(fs.readFileSync(f)).split(/\r?\n/), []); } catch (e) { /* 忽略 */ }
  }
  return result;
}
...
async function genZoneData(engineRoot, commonFolder, zoneFolder) {
  ...
  // 防乱码：iconv-lite 缺失时 lib 会静默回退 UTF-8（GBK 源被误读+误写 → 通区数据乱码）。此处显式拦截，避免生成坏数据。
  if (!iconv) return { ok: false, msg: '缺少 iconv-lite 依赖，无法以 GBK 生成通区数据（请用 GUI 工具箱生成，勿用 CLI）' };
  // 方案B：智能定位 MonItems（monItemsDir 支持 MonItems/MonItemsEx/MonItems_def/MonDrop + 多层级）并校验引擎根有效性；
  // 无效直接报错、不建任何目录（避免引擎根选错产生 D:\通区文件 空壳）
  const itemsSrc = monItemsDir(engineRoot);
  if (!fs.existsSync(itemsSrc)) return { ok: false, msg: '引擎根无效或缺少 MonItems 掉落目录：' + itemsSrc + '（请确认引擎根选到 MirServer 根，而非上级目录）' };
  const zoneDir = path.resolve(engineRoot, '..', commonFolder || '通区文件', zoneFolder);
  // 1. 物品数据（反向索引）：MonItems/<怪物>.txt（新爆率 1/x 物品名）→ 物品 → 掉落它的怪物列表
  //    支持 #CALL 递归（蜗牛/定制体系：MonItems → 通用爆率/地图爆率 → 装备列表，3 层）+ [@段] 花括号界定 + 防循环
  const outItems = path.join(zoneDir, '物品数据');
  let nItems = 0, itemRows = 0, batch = 0;
  const itemToMons = {};
  // ---- #CALL 递归解析：返回该怪掉落的所有物品名（去重，含 #CALL 引用）----
  // 复用共享 #CALL 递归解析（与开区网站一致），取物品名去重
  const resolveMonDrops = (monName) => resolveMonDropsAll(engineRoot, monName).map(x => x.item);
  if (fs.existsSync(itemsSrc)) {
    try { fs.mkdirSync(outItems, { recursive: true }); } catch (e) {}
    // 清空旧数据文件（避免 MonGen/MonItems/脚本变化后残留旧文件，如坏行产物）
    try { for (const f of fs.readdirSync(outItems)) if (/\.txt$/i.test(f)) fs.unlinkSync(path.join(outItems, f)); } catch (e) {}
    for (const f of fs.readdirSync(itemsSrc)) {
      if (!/\.txt$/i.test(f) || /^@/.test(f)) continue;
      const monName = f.replace(/\.txt$/i, '');
      const names = resolveMonDrops(monName);
  ...
  const mgFile = monGenFile(engineRoot); // 智能定位 MonGen.txt（支持多层级）
  ...
  const byMon = {};
  // 地图名规范化（去开头编号/全角化/归约重复）——MonGen 与脚本刷怪共用
  const titleFor = (mapCode) => {
    let title = (mapTitles[mapCode] || mapCode).replace(/[|\t\r\n]+/g, ' ').trim().replace(/ +/g, '　');
    title = title.replace(/^[A-Za-z0-9\-]+\s+/, ''); // 去开头编号："96　帝王古墓二层"→"帝王古墓二层"
    title = title.replace(/^\d+(?=[\u4e00-\u9fa5])/, ''); // 去紧贴中文的纯数字编号："14093海上森林　海上森林"→"海上森林　海上森林"
    const tSegs = title.split('　');
    if (tSegs.length >= 2 && tSegs[tSegs.length - 1] && tSegs.every(s => s === tSegs[tSegs.length - 1])) title = tSegs[tSegs.length - 1];
    return title;
  };
  if (fs.existsSync(mgFile)) {
    try { fs.mkdirSync(outMaps, { recursive: true }); } catch (e) {}
    // 清空旧地图数据文件（避免残留）
    try { for (const f of fs.readdirSync(outMaps)) if (/\.txt$/i.test(f)) fs.unlinkSync(path.join(outMaps, f)); } catch (e) {}
    // 用 readMonGen 解析（支持多 token 怪名、MONGENEX 前缀、tab/空格列差异）——怪名含空格的版本不解析错位
    const mgRows = readMonGen(engineRoot);
    for (const row of (mgRows && mgRows.spawns) || []) {
      const mapCode = String(row.map || '').trim(), monName = String(row.mon || '').trim();
      if (!monName || !mapCode) continue;
      const count = String(row.count != null ? row.count : '0'), time = String(row.time != null ? row.time : '0');
      const mx = String(row.x != null ? row.x : '0'), my = String(row.y != null ? row.y : '0');
      if (!byMon[monName]) byMon[monName] = [];
      // 数据格式：代码|地图名|数量|时间|X|Y（数据5/6=X/Y 供点击传送 @虾米飞走(代码,X,Y)）
      byMon[monName].push(mapCode + '|' + titleFor(mapCode) + '|' + count + '|' + time + '|' + mx + '|' + my);
    }
  }
  // 2.5 动态/脚本刷怪补充：MONGENEX 地图 X Y 怪名 范围 数量（RobotManage 定时刷怪、NPC 脚本、QuestDiary 活动脚本）
  //     这些怪不在 MonGen.txt，通区数据会漏（古迹端事故：世界BOSS/活动怪无刷新地图）。只收固定怪名（含 <> 变量的随机怪跳过）。
  const scriptDirs = [path.join(env, 'Robot_def'), path.join(env, 'Market_def'), path.join(env, 'QuestDiary')];
  const scriptRe = /MONGENEX\s+(\S+)\s+(-?\d+)\s+(-?\d+)\s+(\S+)\s+(\d+)\s+(\d+)/gi;
  const seenScript = new Set();
  let nScript = 0;
  const scanScript = (dir, depth) => {
    if (depth > 2 || !fs.existsSync(dir)) return;
    let items;
    try { items = fs.readdirSync(dir, { withFileTypes: true }); } catch (e) { return; }
    for (const it of items) {
      const full = path.join(dir, it.name);
      if (it.isDirectory()) { scanScript(full, depth + 1); continue; }
      if (!/\.(txt|ini)$/i.test(it.name)) continue;
      let c;
      try { c = decodeBuf(fs.readFileSync(full)); } catch (e) { continue; }
      for (const m of c.matchAll(scriptRe)) {
        const mapCode = m[1], mx = m[2], my = m[3], monName = m[4], count = m[6];
        if (!monName || monName.includes('<') || monName.includes('>')) continue; // 变量怪名（<$str(S3)> 随机抽）跳过
        const key = monName + '|' + mapCode + '|' + mx + '|' + my;
        if (seenScript.has(key)) continue;
        seenScript.add(key);
        if (!byMon[monName]) byMon[monName] = [];
        byMon[monName].push(mapCode + '|' + titleFor(mapCode) + '|' + count + '|0|' + mx + '|' + my);
        nScript++;
      }
    }
  };
  for (const d of scriptDirs) scanScript(d, 0);
  // 2.6 掉落怪名别名文件：咕咕鸡存储界面按"掉落怪名(MonItems 文件名)"读 地图数据/<怪>.txt，
  //     但地图数据按 MonGen/脚本刷怪名生成（带修饰前缀/变体后缀，如 夜不归宿□夜游神 vs 掉落名 夜游神）→ 精确读不到 → 刷新地图空白
  //     这里为每个掉落怪名生成别名文件（内容=匹配到的刷怪数据），界面按掉落名即可读到
  const dropSrc = path.join(env, 'MonItems');
  const normAlias = (s) => String(s || '').replace(/\[[^\]]*\]/g, '').replace(/【[^】]*】/g, '')
    .replace(/[★☆◆◇≮≯『』「」∈∵□Чづ·•‧・·﹒.\-—_~\s]+/g, '').toLowerCase();
  const monKeys = Object.keys(byMon);
  const monNorm = monKeys.map(k => ({ k, n: normAlias(k) }));
  const spawnMatch = (mon) => {
    if (byMon[mon]) return mon;
    const dn = normAlias(mon);
    if (!dn || dn.length < 2) return null;
    let m2 = monNorm.find(x => x.n === dn);
    if (!m2) {
      const cands = monNorm.filter(x => x.n.includes(dn));
      if (cands.length) m2 = cands.sort((a, b) => a.n.length - b.n.length)[0];
    }
    return m2 ? m2.k : null;
  };
  if (fs.existsSync(dropSrc)) {
    let aliasCount = 0;
    for (const f of fs.readdirSync(dropSrc)) {
      if (!/\.txt$/i.test(f) || /^@/.test(f)) continue;
      const dropName = f.replace(/\.txt$/i, '');
      if (byMon[dropName]) continue; // 已有精确文件
      const mk = spawnMatch(dropName);
      if (mk) { byMon[dropName] = byMon[mk]; aliasCount++; }
    }
    if (aliasCount) console.log('[genZoneData] 掉落怪名别名文件: ' + aliasCount + ' 个');
  }
  // 2.7 过滤物品数据：掉落怪列表只保留有刷新地图的怪（MonGen 被注释/无刷怪配置的怪不显示，避免玩家点到空白误解）
  if (fs.existsSync(outItems)) {
    let filteredFiles = 0, removedRows = 0;
    for (const f of fs.readdirSync(outItems)) {
      if (!/\.txt$/i.test(f)) continue;
      const p = path.join(outItems, f);
      let c = '';
      try { c = decodeBuf(fs.readFileSync(p)); } catch (e) { continue; }
      const lines = c.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
      const kept = lines.filter(m => spawnMatch(m));
      removedRows += lines.length - kept.length;
      if (kept.length !== lines.length) {
        try { fs.writeFileSync(p, encodeStr(kept.join('\r\n'), 'gbk')); filteredFiles++; } catch (e) {}
      }
    }
    if (filteredFiles) console.log('[genZoneData] 物品数据过滤无刷新怪: ' + filteredFiles + ' 文件, 剔除 ' + removedRows + ' 行');
  }
  // 统一写地图数据（MonGen 常规刷怪 + 脚本动态刷怪合并）
  ...
  return { ok: true, items: nItems, itemRows, maps: nMaps, mapRows, msg: '通区数据：物品数据 ' + nItems + ' 怪(' + itemRows + ' 行)、地图数据 ' + nMaps + ' 怪(' + mapRows + ' 行，其中脚本刷怪补充 ' + nScript + ' 条)' };
}
...
// Market_Def / Market_def 目录名大小写兼容（Windows 不敏感但保险）
const marketDefDir = (env) => {
  if (fs.existsSync(path.join(env, 'Market_Def'))) return 'Market_Def';
  if (fs.existsSync(path.join(env, 'Market_def'))) return 'Market_def';
  return 'Market_Def';
};

async function genStoreScript(engineRoot, opts = {}) {
  // 引擎检测提示：模板用翎风命令（GetBagInfo/FORMULATION 等），GOM 引擎需确认兼容（快餐服换端防翻车）
  let engineLog = '';
  try {
    const eng = detectEngine(engineRoot);
    if (eng && eng.engine && /GOM/i.test(eng.engine)) engineLog = '；⚠ 检测到 ' + eng.engine + ' 引擎，存销模板含翎风命令（GetBagInfo/FORMULATION/ExtractStringEx），请确认该引擎兼容，否则需适配模板';
  } catch (e) { /* 忽略 */ }
  ...
  const env = path.join(engineRoot, 'Mir200', 'Envir');
  // 方案B：智能校验引擎根（MonItems 智能定位），无效直接报错不建目录（防引擎根选错产生空壳）
  const miDir2 = monItemsDir(engineRoot);
  if (!fs.existsSync(miDir2)) return { ok: false, msg: '引擎根无效或缺少 MonItems 掉落目录：' + miDir2 + '（请确认引擎根选到 MirServer 根，而非上级目录）' };
  ...
  if (fs.existsSync(qm)) {
    ...
    // 段名大小写不敏感（引擎 [@Login]/[@login] 视为同一段）：已有段则合并进段内，避免重复 Login 段（古迹端事故）
    const loginRe = /\[@login\]/i;
    const loginMatch = c.match(loginRe);
    if (loginMatch) {
      const nl = c.indexOf('\n', loginMatch.index);
  ...
  const qf = path.join(env, marketDefDir(env), 'QFunction-0.txt');
  ...
  return { ok: true, timerId, qm: qmLog, qf: qfLog, files: written, effect: effectLog, cats: catRes, zone: zoneRes, hook: hookRes, msg: '已生成 ' + written.length + ' 个商店脚本 + ' + qmLog + ' + ' + qfLog + ' + ' + effectLog + catLog + zoneLog + hookLog + engineLog };
}
...
async function storeCleanScripts(engineRoot, opts = {}) {
  ...
  const targets = [
    ...
    path.join(env, marketDefDir(env), 'QFunction-0.txt'),
  ];
  const patterns = [
  ...
  for (const f of targets) {
    ...
    c = c.replace(/^\[@[^\]]+\][ \t]*(?:\r?\n[ \t]*)*(?=\[@)/gm, ''); // 清理剥离标记后残留的空段头（旧版追加 [@Login] 事故）
    c = c.replace(/\n{3,}/g, '\n\n');
...
function exportSiteData(engineRoot) {
  ...
  try {
    ...
    for (const raw of mi.split(/\r?\n/)) {
      ...
      // 传送点解析：代码可为中文/ASCII（\S+），坐标支持逗号(X,Y)或空格(X Y)两种写法（怀念端逗号、古迹端空格）
      const t = line.match(/^(\S+)\s+(-?\d+)[,\s](-?\d+)\s*->\s*(\S+)\s+(-?\d+)[,\s](-?\d+)/);
  ...
  // 动态/脚本刷怪补充（MONGENEX 地图 X Y 怪名 范围 数量）：世界BOSS/活动怪不在 MonGen，补进网站刷新数据
  const scriptDirs2 = [path.join(env, 'Robot_def'), path.join(env, 'Market_def'), path.join(env, 'QuestDiary')];
  const scriptRe2 = /MONGENEX\s+(\S+)\s+(-?\d+)\s+(-?\d+)\s+(\S+)\s+(\d+)\s+(\d+)/gi;
  const seenScript2 = new Set();
  const scanSiteScript = (dir, depth) => {
    if (depth > 2 || !fs.existsSync(dir)) return;
    let items;
    try { items = fs.readdirSync(dir, { withFileTypes: true }); } catch (e) { return; }
    for (const it of items) {
      const full = path.join(dir, it.name);
      if (it.isDirectory()) { scanSiteScript(full, depth + 1); continue; }
      if (!/\.(txt|ini)$/i.test(it.name)) continue;
      let c;
      try { c = decodeBuf(fs.readFileSync(full)); } catch (e) { continue; }
      for (const m of c.matchAll(scriptRe2)) {
        const mc = m[1], mx = m[2], my = m[3], monName = m[4], count = m[6];
        if (!monName || monName.includes('<') || monName.includes('>')) continue;
        const key = monName + '|' + mc + '|' + mx + '|' + my;
        if (seenScript2.has(key)) continue;
        seenScript2.add(key);
        if (!spawnsByMon[monName]) spawnsByMon[monName] = [];
        if (spawnsByMon[monName].length < 30) spawnsByMon[monName].push({ map: mc, x: +mx || 0, y: +my || 0, count: +count || 1, time: 0, time_text: '' });
        if (!mapMonsters[mc]) mapMonsters[mc] = new Set();
        if (mapMonsters[mc].size < 60) mapMonsters[mc].add(monName);
      }
    }
  };
  for (const d of scriptDirs2) scanSiteScript(d, 0);
  // 掉落怪名 → 刷怪数据别名匹配（掉落名去变体后缀/装饰后与刷怪名包含匹配，解决 夜游神 vs 夜不归宿□夜游神）
  const normAliasSite = (s) => String(s || '').replace(/\[[^\]]*\]/g, '').replace(/【[^】]*】/g, '')
    .replace(/[★☆◆◇≮≯『』「」∈∵□Чづ·•‧・·﹒.\-—_~\s]+/g, '').toLowerCase();
  const spawnKeysSite = Object.keys(spawnsByMon);
  const spawnNormSite = spawnKeysSite.map(k => ({ k, n: normAliasSite(k) }));
  const spawnsFor = (mon) => {
    if (spawnsByMon[mon]) return spawnsByMon[mon];
    const dn = normAliasSite(mon);
    if (!dn || dn.length < 2) return [];
    let m2 = spawnNormSite.find(x => x.n === dn);
    if (!m2) {
      const cands = spawnNormSite.filter(x => x.n.includes(dn));
      if (cands.length) m2 = cands.sort((a, b) => a.n.length - b.n.length)[0];
    }
    return m2 ? spawnsByMon[m2.k] : [];
  };
    ...
    for (const n of (r && r.npcs) || []) {
      ...
      if (n.script && fs.existsSync(n.script) && npcRoutes.length < 4000) {
        try {
          ...
          // 收集该 NPC 脚本里所有 mapmove（排除 self-loop：目标==NPC所在地图），一个 NPC 可传多个外部地图
          const mmRe = /(?:^|\n)\s*(?:#act\s*)?mapmove\s+(\S+)\s+(-?\d+)\s+(-?\d+)/ig;
          let mm;
          const fromMap0 = String(n.map || '').trim();
          while ((mm = mmRe.exec(sc)) && npcRoutes.length < 4000) {
            const toMap0 = String(mm[1] || '').trim();
            if (!toMap0 || toMap0 === fromMap0) continue; // 排除 self-loop（如 群魔营地 NPC 内部传送）
            npcRoutes.push({
              from: fromMap0, to: toMap0, from_x: +n.x || 0, from_y: +n.y || 0, to_x: +mm[2], to_y: +mm[3],
              from_name: nameOf(fromMap0) || fromMap0, to_name: nameOf(toMap0) || toMap0, via: 'npc', npc: nm
            });
          }
  ...
  // 坐标触发传送（地图走法）：QuestDiary/Market_def 脚本里 CheckHumInRange 地图 X Y → GOTO @段 → 段内 mapmove 目标
  // 如 群魔营地 (268,286) → @地底深渊 → mapmove 地底深渊一层（幽灵地堡入口）——不走 MapInfo/NPC 注册，之前漏收录导致走法空
  try {
    const trigDirs = [path.join(env, 'QuestDiary'), path.join(env, 'Market_def'), path.join(env, 'Market_Def')];
    const seenTrig = new Set();
    const scanTrig = (dir, depth) => {
      if (depth > 2 || !fs.existsSync(dir)) return;
      let items;
      try { items = fs.readdirSync(dir, { withFileTypes: true }); } catch (e) { return; }
      for (const it of items) {
        const full = path.join(dir, it.name);
        if (it.isDirectory()) { scanTrig(full, depth + 1); continue; }
        if (!/\.(txt|ini)$/i.test(it.name)) continue;
        let c;
        try { c = decodeBuf(fs.readFileSync(full)); } catch (e) { continue; }
        const lines = c.split(/\r?\n/);
        ...
        for (const l of lines) {
          const sm = l.trim().match(/^\[@([^\]]+)\]\s*$/);
          if (sm) { curSeg = sm[1]; segs[curSeg] = []; continue; }
          if (curSeg) segs[curSeg].push(l);
        }
        for (let i = 0; i < lines.length; i++) {
          const cm = lines[i].match(/CheckHumInRange\s+(\S+)\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)/i);
          if (!cm) continue;
          for (let j = i + 1; j < Math.min(i + 8, lines.length); j++) {
            const gm = lines[j].match(/GOTO\s+@(\S+)/i);
            if (gm) {
              const segBody = segs[gm[1]] || [];
              for (const sl of segBody) {
                const mm = sl.match(/mapmove\s+(\S+)\s+(-?\d+)\s+(-?\d+)/i);
                if (mm && mm[1] !== cm[1]) {
                  const key = cm[1] + '|' + mm[1] + '|' + cm[2] + '|' + cm[3];
                  if (!seenTrig.has(key) && routes.length < 4000) {
                    seenTrig.add(key);
                    routes.push({ from: cm[1], to: mm[1], from_x: +cm[2], from_y: +cm[3], to_x: +mm[2], to_y: +mm[3],
                      from_name: nameOf(cm[1]) || cm[1], to_name: nameOf(mm[1]) || mm[1], via: 'trigger' });
                  }
                  break;
                }
              }
            ...
            if (lines[j].trim() === '' || /^#IF/i.test(lines[j].trim())) continue;
          }
        }
      }
    };
    for (const d of trigDirs) scanTrig(d, 0);
  } catch (e) { /* 忽略 */ }
  // MonItems：怪掉落（den 换算）
  const files = listMonFiles(engineRoot);
  const monsters = [];
  const itemDrops = {};
  for (const mon of files) {
    // 与存储（genZoneData）一致：#CALL 递归解析（含通用爆率/地图爆率/装备列表，多层嵌套）
    const drops = resolveMonDropsAll(engineRoot, mon);
    const items = drops.map(d => ({ name: d.item, den: d.den, group_den: 1 }));
    for (const d of drops) {
      // 与咕咕鸡存储一致：只收录有刷新地图的怪（无刷怪配置的怪不显示，避免玩家误解）
      if (spawnsFor(mon).length) { (itemDrops[d.item] = itemDrops[d.item] || {})[mon] = d.den; }
    }
    if (items.length) monsters.push({ monster: mon, spawns: spawnsFor(mon), items });
  }
...
// loginDir: 登录器目录（含 pak.txt 等）；clientDir: 新客户端根（含盘符，如 D:\17周年）；patchDir: 新补丁目录（可选）
// patchDir 兼容两种填法：完整路径（D:\cqkhd\CGR）或客户端根下的补丁名（CGR）；留空 = 保留原补丁名
// 规则：每行路径 [盘符:\第一段\其余] →
//   第一段为标准目录(Data/Map/Graphics等) → 新客户端根\第一段\其余（如 D:\Data\X → D:\cqkhd\Data\X）
//   第一段为补丁目录 → 新补丁根\其余（如 D:\CGR\Data\X → D:\cqkhd\CGR\Data\X）
// 登录器配置：换版本一键替换路径前缀（pak/Map/Wav/Wil/Wzl）
// loginDir: 登录器目录（含 pak.txt 等）；clientDir: 新客户端根（含盘符，如 D:\\cqkhd）；patchDir: 新补丁目录（可选，空=保留原补丁名）
// 路径结构识别（兼容两种写法）：
//   1) 无旧根名层：  D:\Data\X            → 新客户端根\Data\X
//   2) 根名=补丁名：  D:\CGR\Data\X       → 新补丁根\Data\X（旧客户端整个当补丁，seg1 等于补丁名）
//   3) 根名+补丁层：  D:\旧根\HlCqDj\Data\X → 新客户端根\新补丁\Data\X（seg1=旧客户端根名，seg2=补丁名）
//   4) 根名+标准目录：D:\旧根\Data\X      → 新客户端根\Data\X
// 补丁名取 patchDir 最后一段（支持完整路径 D:\\cqkhd\\CGR 或补丁名 CGR）；留空=仅换客户端根，其余原样
function replaceLoginCfg(loginDir, opts) {
  ...
  const patchRaw = String(o.patchDir || '').trim().replace(/[\\/]+$/, '');
  const patchRoot = /^[A-Za-z]:[\\/]/.test(patchRaw) ? patchRaw : (patchRaw ? clientDir + '\\' + patchRaw : '');
  const patchName = patchRoot ? path.basename(patchRoot).toLowerCase() : '';
  if (!loginDir || !fs.existsSync(loginDir)) return { ok: false, msg: '登录器目录不存在: ' + loginDir };
  const files = ['pak.txt', 'Map.txt', 'Wav.txt', 'Wil.txt', 'Wzl.txt', 'Data.txt'];
  ...
  const warns = [];
  let skipCount = 0;
  // 方案B（标准目录定位，通用一层/两层客户端根 + 有/无补丁）：
  //   路径里最后一个标准目录段（Data/Map/Graphics 等）为锚点；
  //   锚点前段数 >=2 → 末段=补丁名、之前=旧客户端根（整体替换，兼容龙族 E:\盛大客户端\Legend of mir\XinchenmoFg\Data）；
  //   锚点前段数 ==1 → 无补丁，仅换根（兼容 Wzl 挂客户端根 data）。
  const replacePath = (l) => {
    const pipeIdx = l.indexOf('|');
    const pathPart = (pipeIdx >= 0 ? l.slice(0, pipeIdx) : l).trim();
    const tail = pipeIdx >= 0 ? l.slice(pipeIdx) : '';
    if (!/^[A-Za-z]:\\/.test(pathPart)) return l;
    const segs = pathPart.split('\\');
    if (segs.length < 3) return l;
    let stdIdx = -1;
    for (let i = segs.length - 2; i >= 1; i--) {
      if (STD.has(segs[i].toLowerCase())) { stdIdx = i; break; }
    }
    if (stdIdx < 0) { skipCount++; return l; }
    const rest = segs.slice(stdIdx).join('\\');
    const preCount = stdIdx - 1;
    if (preCount >= 2) {
      return (patchRoot || clientDir) + '\\' + rest + tail;
    }
    return clientDir + '\\' + rest + tail;
  };
  for (const f of files) {
  ...
  if (skipCount) warns.push('有 ' + skipCount + ' 行路径不含标准目录（Data/Map 等），未替换，请核对');
  const warnMsg = warns.length ? '；⚠ ' + warns.join('；') : '';
  return { ok: true, msg: '登录器配置替换完成（客户端=' + clientDir + (patchRoot ? '，补丁=' + patchRoot : '，仅换客户端根') + '）' + warnMsg, stat, bakDir, warns };
}
;
module.exports = {
  replaceLoginCfg,
